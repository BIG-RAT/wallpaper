...
class WriteToLog {
    ...
    var writeToLogQ = DispatchQueue(label: "com.jamfie.writeToLogQ", qos: DispatchQoS.utility)
    let fm          = FileManager()
            ...
            do {
                ...
                do {
                    ...
                    if Int("\(logSize ?? 0)")! > Log.maxSize {
                        ...
                        zipIt(args: "/usr/bin/zip -rm -jj -o \(Log.path!)JamfWallpaper_\(dateStamp) \(Log.path!)\(Log.file)") {
                            (result: String) in
